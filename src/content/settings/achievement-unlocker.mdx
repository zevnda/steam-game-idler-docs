...
* **Purpose:** Choose a time range for when SGI should unlock achievements. The `achievement unlocker` feature will continue to run, but SGI will not unlock achievements outside of that time range. SGI will automatically continue again once inside the selected time range

### Idle games while active
* **Purpose:** While the `achievement unlocker` is running, the current game will be idled for the duration of time it takes to unlock all the available achievements, or until it is manually stopped. By default, games are not idled when unlocking achievements
